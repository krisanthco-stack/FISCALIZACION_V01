...
(function(root,factory){
  ...
  function canonicalIdentityKey(record){const g=record?.general||record||{},folio=normKey(g.folio),finca=normKey(g.finca),plano=normKey(g.plano),derecho=normKey(g.derecho),id=normKey(g.sourceId),tramite=normKey(g.tramite),base=folio||finca;if(base)return`B:${base}|P:${plano}|D:${derecho}`;if(id)return`ID:${id}|P:${plano}|D:${derecho}`;if(tramite)return`T:${tramite}|P:${plano}|D:${derecho}`;return''}
