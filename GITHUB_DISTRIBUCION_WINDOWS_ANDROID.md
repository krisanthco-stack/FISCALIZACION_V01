...
Los dos builds ejecutan `python scripts/vendor_pdfjs.py --required` antes de empaquetar. El script fija **PDF.js 4.10.38 legacy**, intenta descargarlo desde dos proveedores y lo copia a `app/assets/vendor/pdfjs/`.
...
- PDF escaneado: intenta OCR local cuando el motor del dispositivo ofrece `TextDetector`; si no está disponible, el PDF sigue siendo visible pero el OCR automático puede no estar disponible.
...
- Los PDF se visualizan con el lector PDF.js interno.
